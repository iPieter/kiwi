...
def test_signature_and_examples_are_saved_correctly(xgb_model):
    ...
    for signature in (None, infer_signature(xgb_model.inference_dataframe)):
        for example in (None, xgb_model.inference_dataframe.head(3)):
            with TempDir() as tmp:
                path = tmp.path("model")
                kiwi.xgboost.save_model(xgb_model=model,
                                        path=path,
                                        signature=signature,
...
def test_model_load_from_remote_uri_succeeds(xgb_model, model_path, mock_s3_bucket):
    kiwi.xgboost.save_model(xgb_model=xgb_model.model, path=model_path)
    ...
    reloaded_model = kiwi.xgboost.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
...
def test_log_model_calls_register_model(xgb_model):
    ...
    with kiwi.start_run(), register_model_patch, TempDir(chdr=True, remove_on_exit=True) as tmp:
        conda_env = os.path.join(tmp.path(), "conda_env.yaml")
        _mlflow_conda_env(conda_env, additional_pip_deps=["xgboost"])
        kiwi.xgboost.log_model(xgb_model=xgb_model.model, artifact_path=artifact_path,
...
def test_model_save_accepts_conda_env_as_dict(xgb_model, model_path):
    conda_env = dict(kiwi.xgboost.get_default_conda_env())
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    kiwi.xgboost.save_model(xgb_model=xgb_model.model, path=model_path, conda_env=None)
    ...
    assert conda_env == kiwi.xgboost.get_default_conda_env()
