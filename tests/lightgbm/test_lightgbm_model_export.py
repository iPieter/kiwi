...
def test_signature_and_examples_are_saved_correctly(lgb_model):
    ...
    for signature in (None, signature_):
        for example in (None, example_):
            with TempDir() as tmp:
                path = tmp.path("model")
                kiwi.lightgbm.save_model(model, path=path,
                                         signature=signature,
...
def test_model_load_from_remote_uri_succeeds(lgb_model, model_path, mock_s3_bucket):
    kiwi.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path)
    ...
    reloaded_model = kiwi.lightgbm.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
...
def test_log_model_calls_register_model(lgb_model):
    ...
    with kiwi.start_run(), register_model_patch, TempDir(chdr=True, remove_on_exit=True) as tmp:
        ...
        kiwi.lightgbm.log_model(lgb_model=lgb_model.model, artifact_path=artifact_path,
...
def test_model_save_accepts_conda_env_as_dict(lgb_model, model_path):
    conda_env = dict(kiwi.lightgbm.get_default_conda_env())
...
def test_model_log_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    artifact_path = "model"
    with kiwi.start_run():
        kiwi.lightgbm.log_model(lgb_model=lgb_model.model,
                                artifact_path=artifact_path,
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    kiwi.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path, conda_env=None)
    ...
    assert conda_env == kiwi.lightgbm.get_default_conda_env()
