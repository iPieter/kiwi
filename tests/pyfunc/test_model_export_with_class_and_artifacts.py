...
from kiwi.tracking.artifact_utils import get_artifact_uri as utils_get_artifact_uri, \
...
def get_model_class():
    ...
    class CustomSklearnModel(kiwi.pyfunc.PythonModel):
        ...
        def load_context(self, context):
            ...
            self.model = kiwi.sklearn.load_model(model_uri=context.artifacts["sk_model"])
...
def test_model_save_load(sklearn_knn_model, main_scoped_model_class, iris_data, tmpdir):
    ...
    kiwi.sklearn.save_model(sk_model=sklearn_knn_model, path=sklearn_model_path)
    ...
    kiwi.pyfunc.save_model(path=pyfunc_model_path,
                           artifacts={
    ...
    loaded_pyfunc_model = kiwi.pyfunc.load_pyfunc(model_uri=pyfunc_model_path)
...
def test_pyfunc_model_log_load_no_active_run(sklearn_knn_model, main_scoped_model_class, iris_data):
    ...
    with kiwi.start_run():
        kiwi.sklearn.log_model(sk_model=sklearn_knn_model, artifact_path=sklearn_artifact_path)
...
def test_model_load_from_remote_uri_succeeds(
    ...
    loaded_pyfunc_model = kiwi.pyfunc.load_pyfunc(model_uri=model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]))
...
def test_add_to_model_adds_specified_kwargs_to_mlmodel_configuration():
    ...
    model_config = Model()
    kiwi.pyfunc.add_to_model(model=model_config,
    ...
    assert all([item in model_config.flavors[kiwi.pyfunc.FLAVOR_NAME] for item in custom_kwargs])
...
def test_save_model_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    sklearn_model_path = os.path.join(str(tmpdir), "sklearn_model")
    kiwi.sklearn.save_model(sk_model=sklearn_knn_model,
                            path=sklearn_model_path,
                            serialization_format=kiwi.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE)
...
def test_save_model_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    sklearn_model_path = os.path.join(str(tmpdir), "sklearn_model")
    kiwi.sklearn.save_model(sk_model=sklearn_logreg_model, path=sklearn_model_path)
...
def test_save_model_correctly_resolves_directory_artifact_with_nested_contents(
    ...
    class ArtifactValidationModel(kiwi.pyfunc.PythonModel):
        def predict(self, context, model_input):
    ...
    loaded_model = kiwi.pyfunc.load_pyfunc(model_uri=model_path)
...
def test_load_model_with_differing_cloudpickle_version_at_micro_granularity_logs_warning(
    ...
    model_config.flavors[kiwi.pyfunc.FLAVOR_NAME][
        kiwi.pyfunc.model.CONFIG_KEY_CLOUDPICKLE_VERSION] = saver_cloudpickle_version
...
def test_sagemaker_docker_model_scoring_with_default_conda_env(
    ...
    kiwi.pyfunc.save_model(path=pyfunc_model_path,
                           ...
                           python_model=main_scoped_model_class(test_predict),
                           conda_env=_conda_env())
    reloaded_pyfunc = kiwi.pyfunc.load_pyfunc(model_uri=pyfunc_model_path)
