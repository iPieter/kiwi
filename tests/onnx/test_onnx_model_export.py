...
def test_cast_float64_to_float32():
    ...
    df2 = kiwi.onnx._OnnxModelWrapper._cast_float64_to_float32(df, df.columns)
...
def test_model_save_load(onnx_model, model_path, onnx_custom_env):
    import onnx
    import kiwi.onnx
    kiwi.onnx.save_model(onnx_model, model_path, conda_env=onnx_custom_env)

    # Loading ONNX model
    onnx.checker.check_model = mock.Mock()
    kiwi.onnx.load_model(model_path)
...
def test_signature_and_examples_are_saved_correctly(onnx_model, data, onnx_custom_env):
    ...
    for signature in (None, signature_):
        for example in (None, example_):
            with TempDir() as tmp:
                path = tmp.path("model")
                kiwi.onnx.save_model(model, path=path, conda_env=onnx_custom_env,
                                     signature=signature,
...
def test_model_save_load_evaluate_pyfunc_format(onnx_model, model_path, data, predicted):
    ...
    import kiwi.onnx
    x, y = data
    kiwi.onnx.save_model(onnx_model, model_path)
...
def test_model_save_load_multiple_inputs(
    ...
    kiwi.onnx.save_model(onnx_model_multiple_inputs_float64,
...
def test_model_save_load_evaluate_pyfunc_format_multiple_inputs(
    ...
    kiwi.onnx.save_model(onnx_model_multiple_inputs_float64, model_path)
...
def test_pyfunc_representation_of_float32_model_casts_and_evalutes_float64_inputs(
    ...
    kiwi.onnx.save_model(onnx_model_multiple_inputs_float32, model_path)
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    import kiwi.onnx
    kiwi.onnx.save_model(onnx_model=onnx_model, path=model_path, conda_env=onnx_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(onnx_model, model_path):
    import kiwi.onnx
    conda_env = dict(kiwi.onnx.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    kiwi.onnx.save_model(onnx_model=onnx_model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    import kiwi.onnx
    kiwi.onnx.save_model(onnx_model=onnx_model, path=model_path, conda_env=None)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
    conda_env_path = os.path.join(model_path, pyfunc_conf[pyfunc.ENV])
    with open(conda_env_path, "r") as f:
        conda_env = yaml.safe_load(f)

    assert conda_env == kiwi.onnx.get_default_conda_env()
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with kiwi.start_run():
        kiwi.onnx.log_model(onnx_model=onnx_model, artifact_path=artifact_path, conda_env=None)
        model_path = _download_artifact_from_uri("runs:/{run_id}/{artifact_path}".format(
            run_id=kiwi.active_run().info.run_id, artifact_path=artifact_path))
...
def test_pyfunc_predict_supports_models_with_list_outputs(onnx_sklearn_model,  model_path, data):
    ...
    import skl2onnx
    x, y = data
    kiwi.onnx.save_model(onnx_sklearn_model, model_path)
    wrapper = kiwi.pyfunc.load_model(model_path)
