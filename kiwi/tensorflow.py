...
from kiwi.protos.databricks_pb2 import DIRECTORY_NOT_EMPTY
from kiwi.tracking.artifact_utils import _download_artifact_from_uri
from kiwi.utils import keyword_only, experimental
from kiwi.utils.environment import _mlflow_conda_env
from kiwi.utils.file_utils import _copy_file_or_tree
...
from kiwi.entities import Metric
...
def log_model(tf_saved_model_dir, tf_meta_graph_tags, tf_signature_def_key, artifact_path,
    ...
    :param input_example: (Experimental) Input example provides one or several instances of valid
                          ...
                          base64-encoded.
    """
    return Model.log(artifact_path=artifact_path, flavor=kiwi.tensorflow,
...
class __MLflowTfKerasCallback(Callback):
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        if hasattr(opt, '_name'):
            try_mlflow_log(kiwi.log_param, 'optimizer_name', opt._name)
        # Elif checks are if the optimizer is a TensorFlow optimizer rather than a Keras one.
            ...
            opt = opt.optimizer
            try_mlflow_log(kiwi.log_param, 'optimizer_name', type(opt).__name__)
        if hasattr(opt, 'lr'):
    ...
    def on_train_end(self, logs=None):  # pylint: disable=unused-argument
        try_mlflow_log(kiwi.keras.log_model, self.model, artifact_path='model')
...
class __MLflowTfKeras2Callback(Callback):
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        for attribute in config:
            try_mlflow_log(kiwi.log_param, "opt_" + attribute, config[attribute])
...
def _log_artifacts_with_warning(**kwargs):
    try_mlflow_log(kiwi.log_artifacts, **kwargs)
...
def _log_event(event):
    ...
    if not kiwi.active_run():
        ...
        _AUTOLOG_RUN_ID = kiwi.active_run().info.run_id
    ...
    **TensorFlow Core**
    ...
    def _manage_active_run():
        if not kiwi.active_run():
            try_mlflow_log(kiwi.start_run)
            global _AUTOLOG_RUN_ID
            if kiwi.active_run() is not None:  # defensive check in case `mlflow.start_run` fails
                _AUTOLOG_RUN_ID = kiwi.active_run().info.run_id
        yield kiwi.active_run()
        if kiwi.active_run() is not None and kiwi.active_run().info.run_id == _AUTOLOG_RUN_ID:
    ...
    def train(self, *args, **kwargs):
        with _manage_active_run():
            ...
            if len(args) >= 3:
                try_mlflow_log(kiwi.log_param, 'steps', args[2])
                if len(args) >= 4:
                    try_mlflow_log(kiwi.log_param, 'max_steps', args[3])
            if 'steps' in kwargs:
                try_mlflow_log(kiwi.log_param, 'steps', kwargs['steps'])
            if 'max_steps' in kwargs:
                try_mlflow_log(kiwi.log_param, 'max_steps', kwargs['max_steps'])
    ...
    def export_saved_model(self, *args, **kwargs):
        ...
        if not kiwi.active_run():
            global _AUTOLOG_RUN_ID
            if _AUTOLOG_RUN_ID:
                try_mlflow_log(kiwi.start_run, _AUTOLOG_RUN_ID)
            else:
                try_mlflow_log(kiwi.start_run)
                auto_end = True
