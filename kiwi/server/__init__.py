...
from kiwi.server.handlers import get_artifact_handler, STATIC_PREFIX_ENV_VAR, _add_static_prefix
from kiwi.utils.process import exec_cmd
...
if os.getenv(PROMETHEUS_EXPORTER_ENV_VAR):
    from kiwi.server.prometheus_exporter import activate_prometheus_exporter
...
def _build_gunicorn_command(gunicorn_opts, host, port, workers):
    ...
    return ["gunicorn"] + opts + ["-b", bind_address, "-w", "%s" % workers, "kiwi.server:app"]
