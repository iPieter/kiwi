...
def upgrade(url):
    ...
    **always take a backup of your database before running migrations**. The migrations README,
    ...
    """
    engine = kiwi.store.db.utils.create_sqlalchemy_engine(url)
    if kiwi.store.db.utils._is_initialized_before_mlflow_1(engine):
        kiwi.store.db.utils._upgrade_db_initialized_before_mlflow_1(engine)
    kiwi.store.db.utils._upgrade_db(engine)
