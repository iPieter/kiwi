...
import kiwi
import kiwi.sagemaker
from kiwi.sagemaker import DEFAULT_IMAGE_NAME as IMAGE
from kiwi.utils import cli_args
import kiwi.models.docker_utils
...
@click.option("--mode", default=kiwi.sagemaker.DEPLOYMENT_MODE_CREATE,
              help="The mode in which to deploy the application."
                   " Must be one of the following: {mds}".format(
                     mds=", ".join(kiwi.sagemaker.DEPLOYMENT_MODES)))
@click.option("--archive", "-ar", is_flag=True,
              help=("If specified, any SageMaker resources that become inactive (i.e as the"
                    ...
                    " `--async`.".format(
                      mode_replace=kiwi.sagemaker.DEPLOYMENT_MODE_REPLACE)))
@click.option("--instance-type", "-t", default=kiwi.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_TYPE,
              help="The type of SageMaker ML instance on which to deploy the model. For a list of"
                   " supported instance types, see"
                   " https://aws.amazon.com/sagemaker/pricing/instance-types/.")
@click.option("--instance-count", "-c", default=kiwi.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_COUNT,
...
@click.option("--flavor", "-f", default=None,
              help=("The name of the flavor to use for deployment. Must be one of the following:"
                    ...
                    " from the model's available flavors.".format(
                      supported_flavors=kiwi.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS)))
...
def deploy(app_name, model_uri, execution_role_arn, bucket, image_url, region_name, mode, archive,
    ...
    kiwi.sagemaker.deploy(app_name=app_name, model_uri=model_uri,
...
def run_local(model_uri, port, image, flavor):
    ...
    """
    kiwi.sagemaker.run_local(model_uri=model_uri, port=port, image=image, flavor=flavor)
...
def build_and_push_container(build, push, container, mlflow_home):
    ...
    if build:
        ...
        kiwi.models.docker_utils._build_image(
            container,
    ...
    if push:
        kiwi.sagemaker.push_image_to_ecr(container)
