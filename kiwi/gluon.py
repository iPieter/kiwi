...
from kiwi.utils import experimental
from kiwi.utils.autologging_utils import try_mlflow_log
...
def log_model(gluon_model, artifact_path, conda_env=None, registered_model_name=None,
    ...
    """
    Model.log(artifact_path=artifact_path, flavor=kiwi.gluon, gluon_model=gluon_model,
    ...
    class __MLflowGluonCallback(EpochEnd, TrainEnd, TrainBegin):
        ...
        def epoch_end(self, estimator, *args, **kwargs):
            ...
            for metric in estimator.val_metrics:
                metric_name, metric_val = metric.get()
                logs[metric_name] = metric_val
            try_mlflow_log(kiwi.log_metrics, logs, step=self.current_epoch)
            self.current_epoch += 1

        def train_begin(self, estimator, *args, **kwargs):
            try_mlflow_log(kiwi.log_param, "num_layers", len(estimator.net))
            if estimator.max_epoch is not None:
                try_mlflow_log(kiwi.log_param, "epochs", estimator.max_epoch)
            if estimator.max_batch is not None:
                try_mlflow_log(kiwi.log_param, "batches", estimator.max_batch)
            try_mlflow_log(kiwi.log_param, "optimizer_name",
                           type(estimator.trainer.optimizer).__name__)
            if hasattr(estimator.trainer.optimizer, "lr"):
                try_mlflow_log(kiwi.log_param, "learning_rate",
                               estimator.trainer.optimizer.lr)
            if hasattr(estimator.trainer.optimizer, "epsilon"):
                try_mlflow_log(kiwi.log_param, "epsilon",
