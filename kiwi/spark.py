...
from kiwi.utils.uri import is_local_uri, append_to_uri_path
from kiwi.utils.model_utils import _get_flavor_configuration_from_uri
...
def log_model(spark_model, artifact_path, conda_env=None, dfs_tmpdir=None,
    ...
    if is_local_uri(run_root_artifact_uri):
        return Model.log(artifact_path=artifact_path, flavor=kiwi.spark, spark_model=spark_model,
    ...
    with TempDir() as tmp:
        ...
        _save_model_metadata(
            ...
            signature=signature, input_example=input_example)
        kiwi.tracking.fluent.log_artifacts(tmp_model_metadata_dir, artifact_path)
        if registered_model_name is not None:
            kiwi.register_model("runs:/%s/%s" % (run_id, artifact_path), registered_model_name)
...
def autolog():
    ...
    """
    from kiwi import _spark_autologging
